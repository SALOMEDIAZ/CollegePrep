...
const RecipeDetailPage = () => {
  ...
  const { prepLabel, cookLabel } = useMemo(() => getTimeLabels(meal), [meal]);
  const title = useMemo(() => String(meal?.strMeal ?? ""), [meal?.strMeal]);
  const { first, rest } = useMemo(() => titleParts(title), [title]);
  const steps = useMemo(
    () => splitSteps(meal?.strInstructions ?? null),
